...
void RDCTcpSocket::ExitEventLoop(void)
{
...
void RDCTcpSocket::ConnectToHost(const char* ipAddr, unsigned short port)
{
    RDCHostInfo hostInfo(QString(ipAddr), port);
    return this->ConnectToHost(&hostInfo);
}

void RDCTcpSocket::ConnectToHost(const RDCHostInfo* hostInfo)
{
    ...
    addr.sin_port = htons(hostInfo->getPort());
    addr.sin_addr.s_addr = inet_addr(hostInfo->getIPAddress().toLatin1().constData());

    this->m_pBufferEvent = bufferevent_socket_new(
                this->m_pEventBase, -1, BEV_OPT_CLOSE_ON_FREE | BEV_OPT_THREADSAFE);
    if(this->m_pBufferEvent != nullptr )
    {
        this->m_FileDescriptor = bufferevent_getfd(this->m_pBufferEvent);
        void read_call_back(struct bufferevent*, void*);
        void event_dispatched_call_back(struct bufferevent*, short, void*);
        bufferevent_setcb(this->m_pBufferEvent, read_call_back, nullptr, event_dispatched_call_back, (void*)this);
        bufferevent_enable(this->m_pBufferEvent, EV_READ | EV_WRITE | EV_PERSIST);

        if(bufferevent_socket_connect(this->m_pBufferEvent, (const sockaddr*)&addr, sizeof(struct sockaddr)) == -1)
        {
            if(this->m_pEventHandler != nullptr)
                this->m_pEventHandler->onErrorOccured(this, "无法连接到远程服务器");
            return ;
        }
...
void RDCTcpSocket::ListenAtPort(unsigned short port)
{
    ...
    {
        if(this->m_pEventHandler != nullptr)
            this->m_pEventHandler->onErrorOccured(this, "监听端口失败, 请检查端口是否已被占用");
        return ;
    }
